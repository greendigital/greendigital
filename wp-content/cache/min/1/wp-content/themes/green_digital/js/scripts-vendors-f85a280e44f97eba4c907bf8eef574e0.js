...
choppedjs.onEvent=function(eventType,handler,timeout,config){var defaults={name:'unnamed',mute:!1,immediate:!0};this.options=extend({},defaults,config);this.eventType=eventType;this.timeout=timeout;this.handler=handler;this.isExecuteTime=this.options.immediate;this.interval='';var _this=this;init();function init(){$(window)[_this.eventType](function(){_this.isExecuteTime=!0});_this.interval=setInterval(function(){if(_this.isExecuteTime&&!_this.options.mute){try{handler()}catch(err){console.log(err)}
_this.isExecuteTime=!1}},_this.timeout)}};choppedjs.onScroll=function(handler,timeout,options){return new choppedjs.onEvent('scroll',handler,timeout,options)};choppedjs.onResize=function(handler,timeout,options){return new choppedjs.onEvent('resize',handler,timeout,options)};choppedjs.onMousemove=function(handler,timeout,options){return new choppedjs.onEvent('mousemove',handler,timeout,options)}}(jQuery));window.ChopScroll=function(handler,timeout,name){this.timeout=timeout;this.handler=handler;this.name=name||'unnamed';this.isExecuteTime=!0;this.interval='';var _this=this;init();function init(){jQuery(window).scroll(function(){_this.isExecuteTime=!0});_this.interval=setInterval(function(){if(_this.isExecuteTime){try{handler()}catch(err){console.log(err)}
_this.isExecuteTime=!1}},_this.timeout)}};
/**
 * fullPage 2.1.2
...
Plugin.prototype={init:function(){var pageScroll=this.scrollY();this.noscroll=pageScroll===0;this.disable_scroll();$(this.element).addClass(this.effectClassName);if(pageScroll){this.isRevealed=!0;$(this.element).addClass('notrans');$(this.element).addClass('modify')}
var that=this;window.addEventListener('scroll',function(e){that.scrollPage.call(that,e)})},keys:[32,37,38,39,40],docElem:window.document.documentElement,scrollVal:0,isRevealed:!1,noscroll:!1,isAnimating:!1,trigger:$('button.trigger'),preventDefault:function(e){e=e||window.event;if(e.preventDefault)
e.preventDefault();e.returnValue=!1},ie:(function(){var undef,rv=-1;var ua=window.navigator.userAgent;var msie=ua.indexOf('MSIE ');var trident=ua.indexOf('Trident/');if(msie>0){rv=parseInt(ua.substring(msie+5,ua.indexOf('.',msie)),10)}else if(trident>0){var rvNum=ua.indexOf('rv:');rv=parseInt(ua.substring(rvNum+3,ua.indexOf('.',rvNum)),10)}
return((rv>-1)?rv:undef)}()),wheel:function(e){},disable_scroll:function(){window.onmousewheel=document.onmousewheel=this.wheel},enable_scroll:function(){window.onmousewheel=document.onmousewheel=document.onkeydown=document.body.ontouchmove=null},scrollY:function(){return window.pageYOffset||this.docElem.scrollTop},scrollPage:function(){this.scrollVal=this.scrollY();if(this.noscroll&&!this.ie){if(this.scrollVal<0)return!1;window.scrollTo(0,0)}
if($(this.element).hasClass('notrans')){$(this.element).removeClass('notrans');return!1}
if(this.isAnimating){return!1}
if(this.scrollVal<=0&&this.isRevealed){this.toggle(0)}else if(this.scrollVal>0&&!this.isRevealed){this.toggle(1)}},toggle:function(reveal){this.isAnimating=!0;if(reveal){$(this.element).addClass('mk-intro-triggered');$('.'+this.effectClassName).next().next().addClass('mk-intro-triggered'+' '+'page-effect-'+this.options.effect);$('body').addClass('mk-intro-triggered').trigger('page_intro')}else{this.noscroll=!0;this.disable_scroll();$(this.element).removeClass('mk-intro-triggered');$('.'+this.effectClassName).next().next().removeClass('mk-intro-triggered'+' '+'page-effect-'+this.options.effect);$('body').removeClass('mk-intro-triggered').trigger('page_outro')}
var that=this;setTimeout(function(){that.isRevealed=!that.isRevealed;that.isAnimating=!1;if(reveal){that.noscroll=!1;that.enable_scroll()}},1200)}};$.fn[pluginName]=function(options){return this.each(function(){if(!$.data(this,"plugin_"+pluginName)){$.data(this,"plugin_"+pluginName,new Plugin(this,options))}})}})(jQuery,window,document);(function($,window,document,undefined){var pluginName="stickify",defaults={offset:200};function Plugin(element,options){this.element=element;this.options=$.extend({},defaults,options);this._defaults=defaults;this._name=pluginName;this.init()}
...
jQuery.fx.timer=function(timer){if(timer()&&jQuery.timers.push(timer)&&!running){running=!0;animate(timer.elem)}};jQuery.fx.stop=function(){running=!1}}(jQuery,this));(function(a){a.fn.addBack=a.fn.addBack||a.fn.andSelf;a.fn.extend({actual:function(b,l){if(!this[b]){throw '$.actual => The jQuery method "'+b+'" you called does not exist'}
var f={absolute:!1,clone:!1,includeMargin:!1};var i=a.extend(f,l);var e=this.eq(0);var h,j;if(i.clone===!0){h=function(){var m="position: absolute !important; top: -1000 !important; ";e=e.clone().attr("style",m).appendTo("body")};j=function(){e.remove()}}else{var g=[];var d="";var c;h=function(){c=e.parents().addBack().filter(":hidden");d+="visibility: hidden !important; display: block !important; ";if(i.absolute===!0){d+="position: absolute !important; "}
...
h();var k=/(outer)/.test(b)?e[b](i.includeMargin):e[b]();j();return k}})})(jQuery);Swiper.prototype.plugins.progress=function(swiper,params){'use strict';var isH=swiper.params.mode==='horizontal';var wrapperMaxPosition;function initSlides(){for(var i=0;i<swiper.slides.length;i++){var slide=swiper.slides[i];slide.progressSlideSize=isH?swiper.h.getWidth(slide):swiper.h.getHeight(slide);if('offsetLeft' in slide){slide.progressSlideOffset=isH?slide.offsetLeft:slide.offsetTop}else{slide.progressSlideOffset=isH?slide.getOffset().left-swiper.h.getOffset(swiper.container).left:slide.getOffset().top-swiper.h.getOffset(swiper.container).top}}
if(isH){wrapperMaxPosition=swiper.h.getWidth(swiper.wrapper)+swiper.wrapperLeft+swiper.wrapperRight-swiper.width}else{wrapperMaxPosition=swiper.h.getHeight(swiper.wrapper)+swiper.wrapperTop+swiper.wrapperBottom-swiper.height}}
...
function u(a,b,c){function k(g,d,i){if(!(!c.grouped&&g.length)){var j;if(i===!1||e.isArray(i)){j=h.messages[d.key||d]||h.messages["*"];j=j[c.lang]||h.messages["*"].en;(d=j.match(/\$\d/g))&&e.isArray(i)&&e.each(d,function(m){j=j.replace(this,i[m])})}else j=i[c.lang]||i;g.push(j)}}
...
d.type="onFail";l.trigger(d,[g]);d.isDefaultPrevented()||q[c.effect][0].call(f,g,d);return f},reset:function(g){g=g||a;g.removeClass(c.errorClass).each(function(){var d=e(this).data("msg.el");if(d){d.remove();e(this).data("msg.el",null)}}).unbind(c.errorInputEvent||"");return f},destroy:function(){b.unbind(c.formEvent+".V").unbind("reset.V");a.unbind(c.inputEvent+".V").unbind("change.V");return f.reset()},checkValidity:function(g,d){g=g||a;g=g.not(":disabled");if(!g.length)return!0;d=d||e.Event();d.type="onBeforeValidate";l.trigger(d,[g]);if(d.isDefaultPrevented())return d.result;var i=[];g.not(":radio:not(:checked)").each(function(){var m=[],n=e(this).data("messages",m),v=r&&n.is(":date")?"onHide.v":c.errorInputEvent+".v";n.unbind(v);e.each(w,function(){var o=this,s=o[0];if(n.filter(s).length){o=o[1].call(f,n,n.val());if(o!==!0){d.type="onBeforeFail";l.trigger(d,[n,s]);if(d.isDefaultPrevented())return!1;var x=n.attr(c.messageAttr);if(x){m=[x];return!1}else k(m,s,o)}}});if(m.length){i.push({input:n,messages:m});n.trigger("OI",[m]);c.errorInputEvent&&n.bind(v,function(o){f.checkValidity(n,o)})}
if(c.singleError&&i.length)return!1});var j=q[c.effect];if(!j)throw 'Validator: cannot find effect "'+c.effect+'"';if(i.length){f.invalidate(i,d);return!1}else{j[1].call(f,g,d);d.type="onSuccess";l.trigger(d,[g]);g.unbind(c.errorInputEvent+".v")}
...
(function($){var o='waitForImages';$.waitForImages={hasImageProperties:['backgroundImage','listStyleImage','borderImage','borderCornerImage','cursor']};$.expr[':'].uncached=function(a){if(!$(a).is('img[src!=""]')){return!1}
...
$.each(g,function(i,a){var b=d.css(a);var c;if(!b){return!0}
...
$.each(f,function(i,b){var c=new Image();$(c).on('load.'+o+' error.'+o,function(a){n++;k.call(b.element,n,m,a.type=='load');if(n==m){j.call(e[0]);return!1}});c.src=b.src})})}}(jQuery));
/*!
...
if(sY>=start-winH&&sY<=end){fastdom.write(function(){$el.css({'-webkit-transform':'translateY('+ -currentPoint+'px)','-moz-transform':'translateY('+ -currentPoint+'px)','-ms-transform':'translateY('+ -currentPoint+'px)','-o-transform':'translateY('+ -currentPoint+'px)','transform':'translateY('+ -currentPoint+'px)'});if(abb.debug.fastdom){console.log('write-parallax-1')}})}})};$window.on('scroll resize load',layerPosition)}
...
if(!1||!!document.documentMode){return'ie'}
...
return Public})()})(window,navigator)};function ModuleHeader(){(function($){'use strict';abb.modules.header=(function(){var Public={height:height};var $window=$(window);var header=abb.utils.JSONLookup('theme_header'),adminbar=0,totalHeight=0;function height(){if(php.hasAdminbar){if($window.width()>782){adminbar=32}else{adminbar=46}}
totalHeight=adminbar+(header[0]!=undefined?header[0].params.stickyHeight:0);return Math.round(totalHeight)}
return Public})()})(jQuery)};function ModulePageSection(){(function($){'use strict';abb.modules.page_section=(function(){var Public={};var pageSection=abb.utils.JSONLookup('page_section');function manipulateDOM(){setTimeout(function(){if(pageSection.length){for(var i=0,x=pageSection.length;i<x;i++){var params=pageSection[i].params,id=params.id;if(params.hasBgLayer&&params.bgAttachment==='fixed'&&abb.utils.browser==='chrome'){var height=abb.utils.viewport.y;$('#'+id+' .background-layer').css({'background-attachment':'scroll','height':height+'px'});abb.scroll.parallax('#'+id+' .background-layer',1)}}}},100)}
return manipulateDOM()})()})(jQuery)};(function(){'use strict';abb.modules={};window.onload=function(){Utils();Events();Polyfills();Scroll();ModuleHeader();ModulePageSection()}
abb.debug={};abb.debug.fastdom=!1})();function is_touch_device(){return('ontouchstart' in document.documentElement)}
jQuery.exists=function(selector){return($(selector).length>0)};var isMobile={Android:function(){var check;if(navigator.userAgent.match(/Android/i))check=!0;return check},BlackBerry:function(){var check;if(navigator.userAgent.match(/BlackBerry/i))check=!0;return check},iOS:function(){var check;if(navigator.userAgent.match(/iPhone|iPad|iPod/i))check=!0;return check},Opera:function(){var check;if(navigator.userAgent.match(/Opera Mini/i))check=!0;return check},Windows:function(){var check;if(navigator.userAgent.match(/IEMobile/i)||navigator.userAgent.match(/WPDesktop/i))check=!0;return check},any:function(){return(isMobile.Android()||isMobile.BlackBerry()||isMobile.iOS()||isMobile.Opera()||isMobile.Windows())}};var scrollY=(window.pageYOffset!==undefined)?window.pageYOffset:(document.documentElement||document.body.parentNode||document.body).scrollTop,global_window_width=$(window).width(),global_window_height=$(window).height(),global_admin_bar,global_admin_bar_height=0;$(window).load(function(){if($.exists("#wpadminbar")){global_admin_bar=$("#wpadminbar");global_admin_bar_height=global_admin_bar.height()}});function mk_update_globals(){global_window_width=$(window).width();global_window_height=$(window).height()}
...
function mk_one_page_scroller(){"use strict";$('.mk-edge-one-pager').each(function(){var $this=$(this),$tooltip_txt=[];$this.find('.section').each(function(){$tooltip_txt.push($(this).attr('data-title'))});$this.fullpage({verticalCentered:!1,resize:!0,slidesColor:['#ccc','#fff'],anchors:$tooltip_txt,scrollingSpeed:600,easing:'easeInQuart',menu:!1,navigation:!0,navigationPosition:'right',navigationTooltips:!1,slidesNavigation:!0,slidesNavPosition:'bottom',loopBottom:!1,loopTop:!1,loopHorizontal:!0,autoScrolling:!0,scrollOverflow:!1,css3:!0,paddingTop:0,paddingBottom:0,normalScrollElements:'#mk-header, #mk-responsive-wrap',normalScrollElementTouchThreshold:5,keyboardScrolling:!0,touchSensitivity:15,continuousVertical:!1,animateAnchor:!0,onLeave:function(index,nextIndex,direction){$('#mk-header.transparent-header').removeClass('light-header-skin dark-header-skin').addClass($this.find('.one-pager-slide').eq(nextIndex-1).attr('data-header-skin')+'-header-skin');$('#fullPage-nav').removeClass('light-skin dark-skin').addClass($this.find('.one-pager-slide').eq(nextIndex-1).attr('data-header-skin')+'-skin')},afterRender:function(){setTimeout(function(){$('#mk-header.transparent-header').removeClass('light-header-skin dark-header-skin').addClass($this.find('.one-pager-slide').eq(0).attr('data-header-skin')+'-header-skin');$('#fullPage-nav').removeClass('light-skin dark-skin').addClass($this.find('.one-pager-slide').eq(0).attr('data-header-skin')+'-skin')},300)},})})}
function mk_one_pager_resposnive(){"use strict";$('.mk-edge-one-pager').each(function(){var $this=$(this),$header_height=0;var $window_height=$(window).outerHeight();if($(window).width()<=1165){$header_height=$('#mk-header').data('height')}
$this.find('.one-pager-slide').each(function(){var $this=$(this),$content=$this.find('.edge-slide-content');if($this.hasClass('left_center')||$this.hasClass('center_center')||$this.hasClass('right_center')){var $this_height_half=$content.outerHeight()/2,$window_half=$window_height/2;$content.css('marginTop',($window_half-$this_height_half-$header_height/2))}
...
ticking=!1}
animationSet();var requestTick=function(){if(!ticking){requestAnimationFrame(animationSet);ticking=!0}};var rafPolyfill=function(){var lastTime=0;var vendors=['ms','moz','webkit','o'];for(var x=0;x<vendors.length&&!window.requestAnimationFrame;++x){window.requestAnimationFrame=window[vendors[x]+'RequestAnimationFrame'];window.cancelAnimationFrame=window[vendors[x]+'CancelAnimationFrame']||window[vendors[x]+'CancelRequestAnimationFrame']}
if(!window.requestAnimationFrame)
...
function mk_page_section_social_video_bg(){$(".mk-page-section.social-hosted").each(function(){var $container=$(this),$source=$container.data('source'),player;if($source=='youtube'){var youtube=$container.find('iframe')[0];player=new YT.Player(youtube);setTimeout(function(){player.playVideo()},1000)}
if($source=='vimeo'){var vimeo=$container.find('iframe')[0];player=$f(vimeo);setTimeout(function(){player.api('play')},1000)}})};function loop_audio_init(){};function mk_lightbox_init(){$(".mk-lightbox").fancybox({padding:15,margin:15,width:800,height:600,minWidth:100,minHeight:100,maxWidth:9999,maxHeight:9999,pixelRatio:1,autoSize:!0,autoHeight:!1,autoWidth:!1,autoResize:!0,fitToView:!0,aspectRatio:!1,topRatio:0.5,leftRatio:0.5,scrolling:'auto',wrapCSS:'',arrows:!0,closeBtn:!0,closeClick:!1,nextClick:!1,mouseWheel:!0,autoPlay:!1,playSpeed:3000,preload:3,modal:!1,loop:!0,openEffect:'fade',openSpeed:200,openEasing:'swing',openOpacity:!0,openMethod:'zoomIn',closeEffect:'fade',closeSpeed:200,closeEasing:'swing',closeOpacity:!0,closeMethod:'zoomOut',nextEffect:'none',nextSpeed:350,nextEasing:'swing',nextMethod:'changeIn',prevEffect:'none',prevSpeed:350,prevEasing:'swing',prevMethod:'changeOut',helpers:{media:{}},tpl:{wrap:'<div class="fancybox-wrap" tabIndex="-1"><div class="fancybox-skin"><div class="fancybox-outer"><div class="fancybox-inner"></div></div></div></div>',image:'<img class="fancybox-image" src="{href}" alt="" />',error:'<p class="fancybox-error">The requested content cannot be loaded.<br/>Please try again later.</p>',closeBtn:'<a title="Close" class="fancybox-item fancybox-close" href="javascript:;"><i class="mk-moon-close-2"></i></a>',next:'<a title="Next" class="fancybox-nav fancybox-next" href="javascript:;"><span><i class="mk-jupiter-icon-arrow-right"></i></span></a>',prev:'<a title="Previous" class="fancybox-nav fancybox-prev" href="javascript:;"><span><i class="mk-jupiter-icon-arrow-left"></i></span></a>',loading:'<div id="fancybox-loading"><div></div></div>'},})};function mk_event_countdown(){if($.exists('.mk-event-countdown')){$('.mk-event-countdown').each(function(){var $this=$(this),$date=$this.attr('data-date'),$offset=$this.attr('data-offset');$this.downCount({date:$date,offset:$offset})})}};function mk_flexslider_init(){$('.mk-flexslider.mk-script-call').each(function(){if($(this).parents('.mk-tabs').length||$(this).parents('.mk-accordion').length){$(this).removeData("flexslider")}
var $this=$(this),$selector=$this.attr('data-selector'),$animation=$this.attr('data-animation'),$easing=$this.attr('data-easing'),$direction=$this.attr('data-direction'),$smoothHeight=$this.attr('data-smoothHeight')=="true"?!0:!1,$slideshowSpeed=$this.attr('data-slideshowSpeed'),$animationSpeed=$this.attr('data-animationSpeed'),$controlNav=$this.attr('data-controlNav')=="true"?!0:!1,$directionNav=$this.attr('data-directionNav')=="true"?!0:!1,$pauseOnHover=$this.attr('data-pauseOnHover')=="true"?!0:!1,$isCarousel=$this.attr('data-isCarousel')=="true"?!0:!1,$arrowLeft=$this.attr('data-arrow-left'),$arrowRight=$this.attr('data-arrow-right');if($arrowLeft==undefined){$arrowLeft='<i class="mk-jupiter-icon-arrow-left"></i>'}else{$arrowLeft='<i class="'+$arrowLeft+'"></i>'}
...
if($.exists('#mk-responsive-nav')){$header_height+=$('.mk-header-inner').height();var nav_height=$window_height-$header_height;$('#mk-responsive-nav').wrap('<div id="mk-responsive-wrap" style="max-height:'+nav_height+'px"></div>');setTimeout(function(){document.getElementById("mk-responsive-wrap").addEventListener('touchstart',function(event){})},300)}}}}
function mk_sidebar_navigation(){"use strict";var $header=$('#mk-header'),$sticky_style=$header.attr('data-header-style');if($sticky_style==3){$('.sidedash-navigation-ul > li').each(function(){var $this=$(this);$this.children('ul').siblings('a').after('<span class="mk-moon-arrow-down mk-nav-arrow mk-nav-sub-closed"></span>')});$('.mk-nav-arrow').stop(!0).on('click',function(e){var $this=$(this);if($this.hasClass('mk-nav-sub-closed')){$this.siblings('ul').slideDown(450).end().removeClass('mk-nav-sub-closed').addClass('mk-nav-sub-opened')}else{$this.siblings('ul').slideUp(450).end().removeClass('mk-nav-sub-opened').addClass('mk-nav-sub-closed')}
...
$('.mk-blog-showcase ul li').each(function(){$(this).on('hover',function(){$(this).siblings('li').removeClass('mk-blog-first-el').end().addClass('mk-blog-first-el')})})};function mk_header_scripts(){if($(window).width()<mk_responsive_nav_width||!($.exists('#mk-header .mk-header-holder')))return!1;var $header=$('#mk-header'),$sticky_style=$header.attr('data-sticky-style'),$sticky_offset=$header.attr('data-sticky-offset'),$header_style=$header.attr('data-header-style'),$ChopScroll_speed=100;if($header_style!=2){if($sticky_offset=='header'){$sticky_offset=parseInt($header.attr('data-height'))}else{$sticky_offset=String($sticky_offset);$sticky_offset=$sticky_offset}}else{var wp_admin_height=$header_toolbar_h=0;if($.exists('.mk-header-toolbar')){$header_toolbar_h=$('.mk-header-toolbar').outerHeight()}
...
function mk_charts(){"use strict";if($.exists('.mk-chart')){$(window).on("load",function(){$('.mk-chart').each(function(){var $this,$parent_width,$chart_size;$this=$(this);$parent_width=$(this).parent().width();$chart_size=$this.attr('data-barSize');if($parent_width<$chart_size){$chart_size=$parent_width;$this.css('line-height',$chart_size);$this.find('i').css({'line-height':$chart_size+'px'})}
if(!$this.hasClass('chart-animated')){$this.easyPieChart({animate:1300,lineCap:'butt',lineWidth:$this.attr('data-lineWidth'),size:$chart_size,barColor:$this.attr('data-barColor'),trackColor:$this.attr('data-trackColor'),scaleColor:'transparent',onStep:function(value){this.$el.find('.chart-percent span').text(Math.ceil(value))}})}})})}};function mk_main_nav_scroll(){"use strict";var lastId,topMenu=$("#mk-main-navigation, .mk-vm-menu"),menuItems=topMenu.find(".menu-item a");menuItems.each(function(){var href_attr=$(this).attr('href');if(typeof href_attr!=='undefined'&&href_attr!==!1){var href=$(this).attr("href").split('#')[0];$(this).addClass("one-page-nav-item")}else{href=""}
...
var offsetTop=0;onePageMenuItems.click(function(e){var href=$(this).attr("href");if(typeof $(href).offset()!='undefined'){var href_top=$(href).offset().top}else{var href_top=0}
if($.exists("#wpadminbar")){var wp_admin_height=$("#wpadminbar").height()}else{wp_admin_height=0}
if($(window).width()<=mk_responsive_nav_width){header_height=0}
offsetTop=href==="#"?0:href_top-(wp_admin_height+header_height-1);$('html, body').stop().animate({scrollTop:offsetTop},{duration:1200,easing:"easeInOutExpo"});e.preventDefault()});function hashLink(){var hash=window.location.hash;if(hash.length){setTimeout(function(){window.scrollTo(0,0)},1);$(window).load(function(){if($.exists("#wpadminbar")){var wp_admin_height=$("#wpadminbar").height()}else{wp_admin_height=0}
var $target=$('#'+hash.substring(1)),offsetHash=$target.offset().top,offset;offset=offsetHash-(wp_admin_height+header_height-1);setTimeout(function(){$('html, body').stop().animate({scrollTop:offset},{duration:1200,easing:"easeInOutExpo"})},500)})}}
hashLink();new ChopScroll(function(){if(!scrollItems.length)return!1;var fromTop=$(window).scrollTop()+(wp_admin_height+header_height);var cur=scrollItems.map(function(){if($(this).offset().top-200<fromTop)return this});cur=cur[cur.length-1];var id=cur&&cur.length?cur[0].id:"";if(lastId!==id){lastId=id;onePageMenuItems.parent().removeClass("current-menu-item current-menu-ancestor current-menu-parent");if(id.length){onePageMenuItems.filter("[href=#"+id+"]").parent().addClass("current-menu-item")}}},200)}
mk_main_nav_scroll();function mk_swipe_slider(){"use strict";$('.mk-swiper-slider').each(function(){if($(this).data('state')!='init'){$(this).data('state','init');var $this=$(this),$thumbs=$this.parent().siblings('.gallery-thumbs-small'),$next_arrow=$this.find('.mk-swiper-next'),$prev_arrow=$this.find('.mk-swiper-prev'),$direction=$this.attr('data-direction'),$pagination=$this.attr('data-pagination')=="true"?!0:!1,$slideshowSpeed=$this.attr('data-slideshowSpeed'),$animationSpeed=$this.attr('data-animationSpeed'),$controlNav=$this.attr('data-controlNav')=="true"?!0:!1,$directionNav=$this.attr('data-directionNav')=="true"?!0:!1,$freeModeFluid=$this.attr('data-freeModeFluid')=="true"?!0:!1,$freeMode=$this.attr('data-freeMode')=="true"?!0:!1,$mousewheelControl=$this.attr('data-mousewheelControl')=="true"?!0:!1,$loop=$this.attr('data-loop')=="true"?!0:!1,$slidesPerView=$this.attr('data-slidesPerView'),vieportWidth=$(window).width();if($pagination===!0){var $pagination_class='#'+$this.attr('id')+' .swiper-pagination'}else{var $pagination_class=!1}
...
var mk_swiper=$(this).swiper({mode:$direction,loop:$loop,freeMode:$freeMode,pagination:$pagination_class,freeModeFluid:$freeModeFluid,autoplay:$slideshowSpeed,speed:$animationSpeed,calculateHeight:!0,grabCursor:!0,useCSS3Transforms:!1,mousewheelControl:$mousewheelControl,paginationClickable:!0,slidesPerView:slidesOnInit,resizeReInit:!0,onSwiperCreated:function(swiper){},onSlideChangeStart:function(){$thumbs.find('.active-item').removeClass('active-item');$thumbs.find('a').eq(mk_swiper.activeIndex).addClass('active-item')}});$prev_arrow.click(function(e){mk_swiper.swipePrev()});$next_arrow.click(function(e){mk_swiper.swipeNext()});$thumbs.find('a').on('touchstart mousedown',function(e){e.preventDefault();$thumbs.find('.active-item').removeClass('active-item');$(this).addClass('active-item');mk_swiper.swipeTo($(this).index())});$thumbs.find('a').click(function(e){e.preventDefault()});if($slidesPerView!='auto'&&$slidesPerView>2){$(window).on("resize",function(){vieportWidth=$(window).width();if(vieportWidth>768){mk_swiper.params.slidesPerView=$slidesPerView;setTimeout(function(){mk_swiper.reInit();setTimeout(mk_swiper.swipePrev(),200)},200)}
...
var mk_swiper=jQuery(this).swiper({mode:$direction,loop:$loop,freeMode:$freeMode,pagination:$pagination_class,freeModeFluid:$freeModeFluid,autoplay:$slideshowSpeed,speed:$animationSpeed,calculateHeight:!0,grabCursor:!0,useCSS3Transforms:!1,mousewheelControl:$mousewheelControl,paginationClickable:!0,slidesPerView:$slidesPerView,onSwiperCreated:function(swiper){},onSlideChangeStart:function(){$thumbs.find('.active-item').removeClass('active-item');$thumbs.find('a').eq(mk_swiper.activeIndex).addClass('active-item')}});$prev_arrow.click(function(e){mk_swiper.swipePrev()});$next_arrow.click(function(e){mk_swiper.swipeNext()});$thumbs.find('a').on('touchstart mousedown',function(e){e.preventDefault();$thumbs.find('.active-item').removeClass('active-item');jQuery(this).addClass('active-item');mk_swiper.swipeTo($(this).index())});$thumbs.find('a').click(function(e){e.preventDefault()})}})}
function mk_edge_parallax(){"use strict";if(!isMobile.any()){var $parallaxLayer=[];$('.mk-edge-wrapper .mk-edge-slider').each(function(){var progressVal,currentPoint,scrollY=(window.pageYOffset!==undefined)?window.pageYOffset:(document.documentElement||document.body.parentNode||document.body).scrollTop,$this=$(this),$window=$(window),windowHeight=$(window).height(),parentHeight=$this.outerHeight(),startPoint=0,endPoint=$this.offset().top+parentHeight,effectLayer=$this,cntLayer=$this.find('.mk-grid'),height=$this.outerHeight();var parallaxSpeed=.7;var animationSet=function(){fastdom.read(function(){scrollY=abb.scroll.Y();currentPoint=(startPoint+scrollY)*parallaxSpeed;progressVal=(1/(endPoint-startPoint)*(scrollY-startPoint));if(abb.debug.fastdom){console.log('read-parallax-2')}
fastdom.write(function(){if(progressVal<=1){effectLayer.css({'-webkit-transform':'translateY('+currentPoint+'px)','-moz-transform':'translateY('+currentPoint+'px)','-ms-transform':'translateY('+currentPoint+'px)','-o-transform':'translateY('+currentPoint+'px)','transform':'translateY('+currentPoint+'px)'})}
cntLayer.css({opacity:(1-(progressVal*2))});if(abb.debug.fastdom){console.log('write-parallax-2')}})})}
animationSet();$window.on('scroll',animationSet)})}};function mobilecheck(){var check=!1;(function(a){if(/(android|ipad|playbook|silk|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(a)||/1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(a.substr(0,4)))check=!0})(navigator.userAgent||navigator.vendor||window.opera);return check}
function mk_click_events(){"use strict";var eventtype=mobilecheck()?'touchstart':'click';jQuery(".mk-header-login, .mk-header-signup, .mk-side-dashboard, .mk-quick-contact-wrapper, .mk-dashboard-trigger, .blog-share-container, .news-share-buttons, .main-nav-side-search, #mk-fullscreen-search-wrapper, #fullscreen-navigation").on(eventtype,function(event){if(event.stopPropagation){event.stopPropagation()}else if(window.event){window.event.cancelBubble=!0}});jQuery("html").on(eventtype,function(){jQuery(this).find(".mk-login-register, #mk-header-subscribe, #mk-quick-contact, .single-share-buttons, .single-share-box, .blog-social-share, .news-share-buttons, #mk-nav-search-wrapper").fadeOut(100);jQuery('.mk-quick-contact-link').removeClass('quick-contact-active');jQuery('.mk-toggle-trigger').removeClass('mk-toggle-active');jQuery('body').removeClass('dashboard-opened');jQuery('.mk-dashboard-trigger').removeClass('dashboard-active')});jQuery('.mk-fullscreen-search-overlay').on(eventtype,function(){$(this).removeClass('mk-fullscreen-search-overlay-show')});jQuery('.mk-fullscreen-nav-wrapper').on(eventtype,function(){$(this).parent().removeClass('opened');$('.mk-dashboard-trigger').removeClass('fullscreen-active')});jQuery('.mk-forget-password').on(eventtype,function(){jQuery('#mk-forget-panel').siblings().hide().end().show()});jQuery('.mk-create-account').on(eventtype,function(){jQuery('#mk-register-panel').siblings().hide().end().show()});jQuery('.mk-return-login').on(eventtype,function(){jQuery('#mk-login-panel').siblings().hide().end().show()});jQuery('.mk-quick-contact-link').on(eventtype,function(){var $this=jQuery(this),$quickContact=jQuery('#mk-quick-contact');if(!$this.hasClass('quick-contact-active')){$quickContact.addClass('quick-contact-anim').fadeIn(250);$this.addClass('quick-contact-active')}else{$quickContact.removeClass('quick-contact-anim').fadeOut(100);$this.removeClass('quick-contact-active')}
return!1});jQuery('.mk-dashboard-trigger').on(eventtype,function(e){var $this=jQuery(this),$body=jQuery('body'),$fullscreen_box=jQuery('.mk-fullscreen-nav');if($this.hasClass('header_dashboard_style')){if(!$this.hasClass('dashboard-active')){$this.addClass('dashboard-active');$body.addClass('dashboard-opened')}else{$this.removeClass('dashboard-active');$body.removeClass('dashboard-opened')}}else if($this.hasClass('header_fullscreen_style')){if(!$this.hasClass('fullscreen-active')){$this.addClass('fullscreen-active');$fullscreen_box.addClass('opened')}else{$this.removeClass('fullscreen-active');$fullscreen_box.removeClass('opened')}}
e.preventDefault()});$('.mk-fullscreen-nav-close').on(eventtype,function(e){$(this).parent().removeClass('opened');$('.mk-dashboard-trigger').removeClass('fullscreen-active');e.preventDefault()})}
...
return!1})}
function mk_social_share_global(){"use strict";var eventtype=mobilecheck()?'touchstart':'click';jQuery('.twitter-share').on(eventtype,function(){var $this=jQuery(this),$url=$this.attr('data-url'),$title=$this.attr('data-title');window.open('http://twitter.com/intent/tweet?text='+$title+' '+$url,"twitterWindow","height=380,width=660,resizable=0,toolbar=0,menubar=0,status=0,location=0,scrollbars=0");return!1});jQuery('.pinterest-share').on(eventtype,function(){var $this=jQuery(this),$url=$this.attr('data-url'),$title=$this.attr('data-title'),$image=$this.attr('data-image');window.open('http://pinterest.com/pin/create/button/?url='+$url+'&media='+$image+'&description='+$title,"twitterWindow","height=320,width=660,resizable=0,toolbar=0,menubar=0,status=0,location=0,scrollbars=0");return!1});jQuery('.facebook-share').on(eventtype,function(){var $url=jQuery(this).attr('data-url');window.open('https://www.facebook.com/sharer/sharer.php?u='+$url,"facebookWindow","height=380,width=660,resizable=0,toolbar=0,menubar=0,status=0,location=0,scrollbars=0");return!1});jQuery('.googleplus-share').on(eventtype,function(){var $url=jQuery(this).attr('data-url');window.open('https://plus.google.com/share?url='+$url,"googlePlusWindow","height=380,width=660,resizable=0,toolbar=0,menubar=0,status=0,location=0,scrollbars=0");return!1});jQuery('.linkedin-share').on(eventtype,function(){var $this=jQuery(this),$url=$this.attr('data-url'),$title=$this.attr('data-title'),$desc=$this.attr('data-desc');window.open('http://www.linkedin.com/shareArticle?mini=true&url='+$url+'&title='+$title+'&summary='+$desc,"linkedInWindow","height=380,width=660,resizable=0,toolbar=0,menubar=0,status=0,location=0,scrollbars=0");return!1})}})(jQuery,window)