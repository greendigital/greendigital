...
if ( !empty( $email ) ) {
	$social_output .= '<li><a target="_blank" href="mailto:'.antispambot( $email ).'" title="'.__( 'Get In Touch With', 'mk_framework' ).' '.get_the_title().'"><i class="mk-moon-mail"></i></a></li>';
}
if ( !empty( $facebook ) ) {
...
$output .= (!empty($link) ? '<a href="'.$link.'"  class="test">' : '');
$output .= '<img alt="'.get_the_title().'" title="'.get_the_title().'" src="'.get_the_post_thumbnail_url() /*mk_thumbnail_image_gen($image_src, $image_dimension, $image_dimension)*/.'" />';
$output .= (!empty($link) ? '</a>' : '');
