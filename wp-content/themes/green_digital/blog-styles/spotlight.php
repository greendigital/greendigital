    ...
    if ($post_type == '') {
     ...
     if (has_post_thumbnail()) {
            $output .= '<article id="' . get_the_ID() . '" class="mk-blog-spotlight-item spotlight-'.$item_id.' mk-isotop-item '.$mk_column_css.' ' . $post_type . '-post-type">' . "\n";
                ...
                $output .=                do_shortcode( '[mk_button dimension="outline" corner_style="pointed" outline_skin="light" text_color="light" size="medium" align="center" url="'.get_permalink().'"]'.__('CONTINUE LENDO', 'mk_framework').'[/mk_button]' );
                $output .= '        </div>';
                $output .= '    </div>';
                $output .= '</div>';
