...
function blog_classic_style($atts, $current)
{
    ...
    if ($post_type == 'audio') {
        ...
        if (empty($iframe)) {
            ...
            if (has_post_thumbnail()) {
                $output .= '<img class="audio-thumb" alt="' . get_the_title() . '" title="' . get_the_title() . '" src="' . mk_thumbnail_image_gen($image_src, 170, 170) . '" itemprop="image" />';
    ...
    $output .= '<a class="mk-readmore" href="' . get_permalink() . '"><i class="mk-moon-arrow-right-2"></i>' . __('CONTINUE LENDO', 'mk_framework') . '</a>';
    $output .= '<div class="clearboth"></div></div>';
    $output .= '</article>' . "\n\n\n";
    return $output;
}